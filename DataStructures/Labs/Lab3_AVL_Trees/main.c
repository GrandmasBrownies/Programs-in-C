...
struct node *delete(int data, struct node *tree, bool *ht_inc)
{
    ...
    if (data == tree->data) // Found node to delete
    {
        if (tree->left && tree->right) { // 2 children
            bptr = findLargestElement(tree->left);
            tree->data = bptr->data;
            tree->left = delete(bptr->data, tree->left, ht_inc);
        }
            ...
            if (tree->left) {
                ...
                tree = tree->right; // tree gets set to NULL if no child
            }
            free (bptr);
            *ht_inc = TRUE;
        }
        ...
        if (*ht_inc == TRUE)
        {
            ...
            case 1: // Left heavy
                ...
                case 1: // LL Rotation
                    ...
                    tree = bptr;
                    break;
                case 0: // LL Rotation with different balance changes
                    tree->left = bptr->right;
                    bptr->right = tree;
                    tree->balance = 1;
                    bptr->balance = -1;
    ...
    while (1)
    {
        ...
        case 3:
            ...
            if (search(root, data) == NULL) {
                ...
                printf("Deletion on the wrong side of the tree.\n");
                printf("This program only deletes values from the right side of the tree\n");
            }
            else {
        ...
        case 4:
            ...
            printf("Enter your option: ");
            scanf("%d", &data);
            int test1[] = {45, 36, 63, 27, 39, 72, 37, 41};
            int test2[] = {54, 45, 63, 39, 51, 65, 18, 47};
            switch (data)
            {
            case 1:
                for (int i = 0; i < 8; i++) {
                    root = insert(test1[i], root, &ht_inc);
                }
                break;
            case 2:
                for (int i = 0; i < 8; i++) {
                    root = insert(test2[i], root, &ht_inc);
                }
                break;
            default:
                printf("Wrong option\n");
